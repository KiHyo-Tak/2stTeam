...
INSERT INTO ACTEVENTDATE VALUES (ACTEVENTDATE_SQ.NEXTVAL, '2020-04-29', '2020-04-30', 'test', 43);
...
INSERT INTO ACTIVITY_RESERVE (arID, hID, mID, actID, eNO) VALUES (ACTIVITY_RESERVE_SQ.NEXTVAL, 'test', 'rlgy0126', 43, 162);
...
select * from review where mid='rlgy0126';
SELECT NVL(AVG(rGRADE), 0) FROM REVIEW WHERE actID=43 AND rCHECK=1;

-- 후기 수정 id : reviewModfy
UPDATE REVIEW SET rTITLE='수정', rCONTENT='내용', rGRADE=3.0 WHERE rID=1 AND rPW='111';

-- 후기 삭제 id : reviewDelete
DELETE REVIEW WHERE rID=73 AND rPW='1234';

SELECT rID, actID, 
    		(SELECT rlNO FROM REVIEW_LIKE WHERE rID=REVIEW.rID) rlNO, 
    		(SELECT rlCHECK FROM REVIEW_LIKE WHERE rID=REVIEW.rID) rlCHECK 
    		FROM (SELECT ROWNUM RN, R.*  
    		FROM (SELECT * FROM REVIEW WHERE actID=43  ORDER BY rGROUP DESC, rSTEP) R) REVIEW 
    		WHERE RN BETWEEN 1 AND 5;

select * from review_like where actid=43;
select * from review where actid=43;
